...
# Auto-nettoyage robuste des caractères UTF-8 problématiques
if [[ "$1" != "--cleaned" ]]; then
    sed -i 's/\xC2\xA0/ /g' "$0" 2>/dev/null || true
    sed -i 's/\xE2\x80\x82/ /g' "$0" 2>/dev/null || true
    sed -i 's/\xE2\x80\x83/ /g' "$0" 2>/dev/null || true
    sed -i 's/\xE2\x80\x89/ /g' "$0" 2>/dev/null || true
    exec "$0" --cleaned "$@"
fi
