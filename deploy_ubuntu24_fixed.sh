#!/bin/bash
# Script de déploiement NTP Monitor Enterprise - Ubuntu 24.04 CORRIGÉ
# Gère les problèmes Redis et NTP spécifiques à Ubuntu 24.04
...
#!/bin/bash
# Monitoring automatique NTP Monitor Enterprise Ubuntu 24.04
...
# Vérifier chrony au lieu de ntp
if ! systemctl is-active --quiet chrony; then
    echo "$(date): Redémarrage chrony" >> /home/ntp-monitor/logs/monitoring.log
...
validate_deployment() {
    ...
    if mysql -u $MYSQL_USER -p$(grep MYSQL_PASSWORD /root/mysql_credentials.txt | cut -d'=' -f2) -e "USE $MYSQL_DB; SHOW TABLES;" 2>/dev/null | grep -q "user"; then
...
display_final_info() {
    ...
    echo "   - Monitoring: /home/$APP_USER/monitor.sh"
    echo
